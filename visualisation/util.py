...
def blocktrans_cen2side(cen_size):
    ...
    sy = float(cen_size[4])
    sz = float(cen_size[5])
    lx, ly, lz = cx - sx / 2.0, cy - sy / 2.0, cz - sz / 2.0
    hx, hy, hz = cx + sx / 2.0, cy + sy / 2.0, cz + sz / 2.0
...
def center_of_mass(voxels, threshold=0.1):
    ...
    center[0] = (
        np.multiply(voxels_filtered.sum(1).sum(1), np.arange(voxels.shape[0])).sum()
        / total
    )
    center[1] = (
        np.multiply(voxels_filtered.sum(0).sum(1), np.arange(voxels.shape[1])).sum()
        / total
    )
    center[2] = (
        np.multiply(voxels_filtered.sum(0).sum(0), np.arange(voxels.shape[2])).sum()
        / total
    )
...
def downsample(voxels, step, method="max"):
    ...
    if voxels.ndim == 3:
        ...
        if method == "max":
            res = ndimage.maximum(
                voxels, labels=regions, index=np.arange(regions.max() + 1)
            )
        elif method == "mean":
            res = ndimage.mean(
                voxels, labels=regions, index=np.arange(regions.max() + 1)
