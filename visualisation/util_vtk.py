...
def block_generation(cen_size, color):
    ...
    cubeActor.GetProperty().SetDiffuse(0.5)
    ...
    for k in range(dims[2]):
        for j in range(dims[1]):
            for i in range(dims[0]):
                ...
                actors.append(
                    block_generation(
                        [i + 0.5, j + 0.5, k + 0.5, block_size, block_size, block_size],
                        color=(color),
                    )
                )
                counter = counter + 1
...
def visualisation(voxels, threshold, title=None, uniform_size=-1, use_colormap=False, snapshot=False):
    ...
    """
    actors = generate_all_blocks(
        voxels, threshold, uniform_size=uniform_size, use_colormap=use_colormap
    )
